...
static float distancePtSeg2(const int x, const int z,
...
static void simplifyContour(dtTempContour& cont, const float maxError)
{
	...
	for (int i = 0; i < cont.npoly; )
	{
		...
		while (ci != endi)
		{
			float d = distancePtSeg2(cont.verts[ci*4+0], cont.verts[ci*4+2], ax, az, bx, bz);
			if (d > maxd)
			{
				maxd = d;
				maxi = ci;
...
inline int prev4(int i, int n) { return i-1 >= 0 ? i-1 : n-1; }
inline int next4(int i, int n) { return i+1 < n ? i+1 : 0; }
...
inline bool xorb3(bool x, bool y)
{
	return !x ^ !y;
}
...
// line through a to b.
inline bool left3(const unsigned char* a, const unsigned char* b, const unsigned char* c)
{
						  ...
						  const unsigned char* c, const unsigned char* d)
{
	...
	return xorb3(left3(a,b,c), left3(a,b,d)) && xorb3(left3(c,d,a), left3(c,d,b));
}
...
static bool	inCone(int i, int j, int n, const unsigned char* verts, const unsigned short* indices)
{
	...
	const unsigned char* pi1 = &verts[(indices[next4(i, n)] & 0x7fff) * 4];
	const unsigned char* pin1 = &verts[(indices[prev4(i, n)] & 0x7fff) * 4];
	
	// If P[i] is a convex vertex [ i+1 left or on (i-1,i) ].
	if (leftOn(pin1, pi, pi1))
		return left3(pi, pj, pin1) && left3(pj, pi, pi1);
	...
	while (n > 3)
	{
		...
		for (int i = 0; i < n; i++)
		{
			...
			if (indices[i1] & 0x8000)
			{
				...
				const unsigned char* p2 = &verts[(indices[next4(i1, n)] & 0x7fff) * 4];
		...
		*dst++ = indices[i2] & 0x7fff;
		...
		if (diagonal(prev4(i, n), i1, n, verts, indices))
			indices[i] |= 0x8000;
		else
			indices[i] &= 0x7fff;
		
		if (diagonal(i, next4(i1, n), n, verts, indices))
			indices[i1] |= 0x8000;
		else
			indices[i1] &= 0x7fff;
	}
...
inline bool uleft2(const unsigned short* a, const unsigned short* b, const unsigned short* c)
{
							 ...
							 const unsigned short* verts, int& ea, int& eb)
{
	...
	vb = pa[ea];
	vc = pb[(eb+2) % nb];
	if (!uleft2(&verts[va*3], &verts[vb*3], &verts[vc*3]))
		return -1;
