...
inline int prev2(int i, int n) { return i-1 >= 0 ? i-1 : n-1; }
inline int next2(int i, int n) { return i+1 < n ? i+1 : 0; }

inline int area22(const int* a, const int* b, const int* c)
{
...
inline bool xorb2(bool x, bool y)
{
	return !x ^ !y;
}
...
// line through a to b.
inline bool left2(const int* a, const int* b, const int* c)
{
	return area22(a, b, c) < 0;
}

inline bool leftOn2(const int* a, const int* b, const int* c)
{
	return area22(a, b, c) <= 0;
}

inline bool collinear2(const int* a, const int* b, const int* c)
{
	return area22(a, b, c) == 0;
}
...
static bool intersectProp2(const int* a, const int* b, const int* c, const int* d)
{
	// Eliminate improper cases.
	if (collinear2(a,b,c) || collinear2(a,b,d) ||
		collinear2(c,d,a) || collinear2(c,d,b))
		return false;
	
	return xorb2(left2(a,b,c), left2(a,b,d)) && xorb2(left2(c,d,a), left2(c,d,b));
}

// Returns T iff (a,b,c) are collinear and point c lies 
// on the closed segement ab.
static bool between2(const int* a, const int* b, const int* c)
{
	if (!collinear2(a, b, c))
		return false;
...
static bool intersect2(const int* a, const int* b, const int* c, const int* d)
{
	if (intersectProp2(a, b, c, d))
		return true;

	if (between2(a, b, c) || between2(a, b, d) ||
		between2(c, d, a) || between2(c, d, b))
		return true;
	
	return false;
}

static bool vequal2(const int* a, const int* b)
{
...
static bool diagonalie(int i, int j, int n, const int* verts, int* indices)
{
	...
	for (int k = 0; k < n; k++)
	{
		...
		if (!((k == i) || (k1 == i) || (k == j) || (k1 == j)))
		{
			...
			if (vequal2(d0, p0) || vequal2(d1, p0) || vequal2(d0, p1) || vequal2(d1, p1))
				continue;
			
			if (intersect2(d0, d1, p0, p1))
				return false;
		}
	}
	return true;
...
static bool	inCone(int i, int j, int n, const int* verts, int* indices)
{
	...
	const int* pi1 = &verts[(indices[next2(i, n)] & 0x0fffffff) * 4];
	const int* pin1 = &verts[(indices[prev2(i, n)] & 0x0fffffff) * 4];

	// If P[i] is a convex vertex [ i+1 left or on (i-1,i) ].
	if (leftOn2(pin1, pi, pi1))
		return left2(pi, pj, pin1) && left2(pj, pi, pi1);
	// Assume (i-1,i,i+1) not collinear.
	// else P[i] is reflex.
	return !(leftOn2(pi, pj, pi1) && leftOn2(pj, pi, pin1));
}
...
static bool	inConeLoose(int i, int j, int n, const int* verts, int* indices)
{
	...
	// If P[i] is a convex vertex [ i+1 left or on (i-1,i) ].
	if (leftOn2(pin1, pi, pi1))
		return leftOn2(pi, pj, pin1) && leftOn2(pj, pi, pi1);
	// Assume (i-1,i,i+1) not collinear.
	// else P[i] is reflex.
	return !(leftOn2(pi, pj, pi1) && leftOn2(pj, pi, pin1));
}
	...
	while (n > 3)
	{
		...
		for (int i = 0; i < n; i++)
		{
			...
			if (indices[i1] & 0x80000000)
			{
				...
				const int* p2 = &verts[(indices[next2(i1, n)] & 0x0fffffff) * 4];
		...
		if (mini == -1)
		{
			...
			for (int i = 0; i < n; i++)
			{
				...
				{
					const int* p0 = &verts[(indices[i] & 0x0fffffff) * 4];
					const int* p2 = &verts[(indices[next2(i2, n)] & 0x0fffffff) * 4];
					int dx = p2[0] - p0[0];
					int dy = p2[2] - p0[2];
					int len = dx*dx + dy*dy;
		...
		*dst++ = indices[i2] & 0x0fffffff;
		...
		if (diagonal(prev2(i, n), i1, n, verts, indices))
			indices[i] |= 0x80000000;
		else
			indices[i] &= 0x0fffffff;
		
		if (diagonal(i, next2(i1, n), n, verts, indices))
			indices[i1] |= 0x80000000;
		else
