...
inline int prev3(int i, int n) { return i-1 >= 0 ? i-1 : n-1; }
inline int next3(int i, int n) { return i+1 < n ? i+1 : 0; }
...
static void triangulateHull(const int /*nverts*/, const float* verts, const int nhull, const int* hull, const int nin, rcIntArray& tris)
{
	...
	for (int i = 0; i < nhull; i++)
	{
		...
		int pi = prev3(i, nhull);
		int ni = next3(i, nhull);
	...
	while (next3(left, nhull) != right)
	{
		// Check to see if se should advance left or right.
		int nleft = next3(left, nhull);
		int nright = prev3(right, nhull);
